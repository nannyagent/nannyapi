#!/bin/bash
# RHEL/CentOS YUM Update Script (for older versions)
# Supports dry-run mode and package exceptions
# Usage: ./rhel-yum-update.sh [--dry-run] [--exclude package1,package2,...]
...
echo "=== RHEL/CentOS YUM Update Script ==="
echo "Dry Run: $DRY_RUN"
echo "Excluded Packages: ${EXCLUDE_PACKAGES:-none}"
echo ""

# Build exclude options for yum
EXCLUDE_OPTS=""
...
UPDATES=$(yum check-update $EXCLUDE_OPTS -q 2>/dev/null | grep -v "^Loaded plugins" | grep -v "^$" || true)
