...
async function updateInvestigation(episodeId, updates) {
  const updateData = {
    ...updates,
    updated_at: new Date().toISOString()
  };
...
async function getInvestigationByEpisode(episodeId) {
...
async function getInvestigationByID(investigationId) {
  const { data, error } = await supabaseClient.from('investigations').select('*').eq('investigation_id', investigationId);
  if (error) {
    console.error(`Failed to get investigation ${investigationId}:`, error);
    return null;
  }
  return data && data.length > 0 ? data[0] : null;
}
async function updateInvestigationByID(investigationId, updates) {
  const updateData = {
    ...updates,
    updated_at: new Date().toISOString()
  };
  const updateBuilder = await supabaseClient.from('investigations').update(updateData);
  const { data, error } = await updateBuilder.eq('investigation_id', investigationId);
  if (error) {
    console.error(`Failed to update investigation ${investigationId}:`, error);
    return null;
  }
  console.log(`Updated investigation ${investigationId}`, updateData);
  return data;
}
async function completeInvestigation(episodeId, resolutionPlan) {
...
async function handleTensorZeroProxyAPI(req, endpointPath, url) {
...
async function handleInferenceAPI(req, endpointPath, url, agentId) {
  if (req.method !== 'GET') {
    return new Response(JSON.stringify({
...
async function handleEpisodeAPI(req, endpointPath, url, agentId) {
  if (req.method !== 'GET') {
    return new Response(JSON.stringify({
...
async function handleEpisodeInferences(episodeId, url, agentId) {
  // Check rate limiting
  const rateLimit = checkRateLimit(agentId);
  if (!rateLimit.allowed) {
    const resetTimeSeconds = Math.ceil((rateLimit.resetTime - Date.now()) / 1000);
...
async function handleHealthAPI(req, agentId) {
  if (req.method !== 'GET') {
    return new Response(JSON.stringify({
...
async function handleOpenAIProxy(req, openaiPath) {
  ...
  // IMPORTANT: Skip investigation tracking for patch-related requests
  // Patches (dry_run, apply) should NOT create investigation records
  const isPatching = requestBody.metadata?.request_type === 'patch' || requestBody.metadata?.is_patch === true || requestBody.function_name?.includes('patch');
  // Track investigation if this is a chat completion (NOT a patch request)
  if (!isPatching) {
    try {
      if (tensorZeroData && tensorZeroData.episode_id) {
        const episodeId = tensorZeroData.episode_id;
        // IMPORTANT: Check if investigation_id is in request metadata (portal-initiated)
        const investigationIdFromMetadata = requestBody.metadata?.investigation_id;
        if (investigationIdFromMetadata) {
          // Portal-initiated: Update existing investigation with episode_id
          console.log(`[INVESTIGATION_UPDATE] Portal-initiated investigation detected: ${investigationIdFromMetadata}`);
          const existingInvestigation = await getInvestigationByID(investigationIdFromMetadata);
          if (existingInvestigation) {
            console.log(`[INVESTIGATION_UPDATE] Found existing investigation, updating with episode_id: ${episodeId}`);
            await updateInvestigationByID(investigationIdFromMetadata, {
              episode_id: episodeId
            });
          } else {
            console.warning(`[INVESTIGATION_UPDATE] Investigation ${investigationIdFromMetadata} not found, skipping update`);
          }
        } else {
          // Agent-initiated: Check if investigation exists by episode_id
          ...
          if (!existingInvestigation) {
            ...
            await createInvestigation({
              agentId,
              episodeId,
              issue: typeof issue === 'string' ? issue.substring(0, 500) : String(issue).substring(0, 500),
              initiatedBy,
              metadata: {
            ...
            if (tensorZeroData.response_type === 'resolution' && tensorZeroData.resolution_plan) {
              ...
              const assistantMessage = tensorZeroData.choices?.[0]?.message?.content || tensorZeroData.content?.[0]?.text || '';
              if (assistantMessage) {
                try {
                  ...
                  isComplete = requestBody.metadata?.investigation_complete === true || requestBody.metadata?.status === 'completed';
                  if (isComplete) {
...
async function retrieveInferenceById(inferenceId, agentId) {
...
async function retrieveEpisodeById(episodeId, agentId) {
  // TODO: Implement actual ClickHouse query to TensorZero database
  ...
  return {
    id: episodeId,
    inference_count: 3,
    first_inference_time: new Date(Date.now() - 3600000).toISOString(),
...
async function retrieveEpisodeInferences(episodeId, agentId, limit, offset) {
  // TODO: Implement actual ClickHouse query to TensorZero database
  ...
  for(let i = offset; i < Math.min(offset + limit, totalInferences); i++){
    inferences.push({
      ...
      timestamp: new Date(Date.now() - (totalInferences - i) * 300000).toISOString(),
      tags: {
        agent_id: agentId
      }
    });
